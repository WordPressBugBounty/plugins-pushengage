jQuery(function ($) {
    ...
    var wc_product_importer = {
        ...
        bindEvents: function () {
            $(document).on('click', '#pe-wc-importer-send-notification-btn', function (e) {
                e.preventDefault();
        ...
        addNotificationButton: function () {
            ...
            $button.on('click', function () { wc_product_importer.showNotificationModal() });
        ...
        showNotificationModal: function () {
            if (typeof $(this).WCBackboneModal === 'function') {
                ...
                console.error('WCBackboneModal method is not available.');
            }
        },
        ...
        validateNotificationFormData() {
            ...
            $form.find('[required]').each(function () {
                ...
                let errorMsg = $field.data('error-message') || 'This field is required.';
                ...
                if (maxLength && value.length > maxLength) {
                    $field.addClass('error').after(`<p class="error-message">The content cannot be longer than ${maxLength} characters.</p>`);
                    isValid = false;
                }

                if ($field.attr('type') === 'url' && value && !/^https?:\/\//i.test(value)) {
                    $field.addClass('error').after('<p class="error-message">Please enter a valid URL.</p>');
                    isValid = false;
                }
            });
